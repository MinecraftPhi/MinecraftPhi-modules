...
execute store result storage phi:modifyinv whole_inventory[0].Slot byte 1 run scoreboard players remove $phi.modifyinv.slot phitemp 9

# Move item into the inventory section:
data modify storage phi:modifyinv inventory append from storage phi:modifyinv whole_inventory[0]
