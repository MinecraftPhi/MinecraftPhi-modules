...
execute store result storage phi:modifyinv whole_inventory[0].Slot byte 1 run scoreboard players remove $phi.modifyinv.slot phitemp 100

# Move item into the armor section:
data modify storage phi:modifyinv armor append from storage phi:modifyinv whole_inventory[0]
