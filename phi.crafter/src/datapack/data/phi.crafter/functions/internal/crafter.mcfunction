...
data modify storage phi.crafter:temp inventory set from block ~ ~ ~ Items
# If the number of UI items is not the expected number recover any items that were put in the place of the UI items
execute store result score $phi.crafter.numUIItems phitemp run data get storage phi.crafter:temp inventory[{tag:{phi:{crafter:{ui:1b}}}}]
